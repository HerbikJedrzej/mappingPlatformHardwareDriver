...
TEST(NormalMode, scenario){
  ...
  Configuration::Configuration configuration = {0x65, 0xAC, MODE_NORMAL,
    {
      1.3686,
      0.0093263,
      50.209,
  ...
  spiInterfaceData.expectedPosition.x = -125.0;
  spiInterfaceData.expectedPosition.y = 80.0;
  spiInterfaceData.expectedPosition.rotation = 270.0;
  ...
  plot::figure_close();
  plot::figure_size(1200, 780);
  plot::named_plot("Y(X) (mm)", measuredX, measuredY);
  plot::title("Position.");
  plot::legend();
  plot::save("./moduleTestsOutputData/" + std::string(test_info_->test_case_name()) + "_XY.png");
  plot::figure_close();
}
